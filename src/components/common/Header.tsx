...
const Header = () => {
  return (
    <header>
      {/* HEADER T */}
      <section className="h-[100px] flex justify-between items-center relative z-10 w-full">
        {/* LOGO */}
        ...
        <nav className="text-black font-bold text-[15px] mr-[158px]">
          <ul className="flex space-x-5">
            {['Homes', 'Pages', 'Products'].map((item) => (
              <li className="relative group" key={item}>
                <a href="#" className="flex items-center whitespace-nowrap">
                  {item} <span className="material-icons-outlined ml-1">expand_more</span>
                </a>
                <ul className="absolute top-full left-0 hidden group-hover:flex flex-col bg-[#EEEEEE] text-black mt-2 w-[200px] shadow-lg rounded-md z-20">
                  {['A', 'B', 'C'].map((sub, i) => (
                    <li key={i}>
                      <a href="#" className="block px-4 py-2 hover:bg-gray-300">{sub}</a>
                    </li>
                  ))}
                </ul>
              </li>
            ...
            <li>
              <a href="#" className="flex items-center whitespace-nowrap">Contact</a>
            </li>
          </ul>
        </nav>
      ...
      {/* HEADER B */}
      <section className="h-[75px] flex items-center justify-between bg-[#1ABA1A] relative z-0 overflow-visible px-[30px] w-full">
        {/* SEARCH */}
          ...
          <div className="flex bg-white rounded-full overflow-hidden shadow-md">
            ...
            <div className="relative group flex items-center">
              ...
              <ul className="absolute left-0 top-full hidden group-hover:flex flex-col bg-[#EEEEEE] text-black mt-1 w-[180px] shadow-lg rounded-md z-50">
                {['A', 'B', 'C'].map((item, i) => (
                  <li key={i}>
                    <a href="#" className="block px-4 py-2 hover:bg-gray-300">{item}</a>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="flex-1 relative">
              ...
              />
              <span className="material-icons-outlined absolute top-1/2 right-3 transform -translate-y-1/2 text-gray-500">search</span>
            </div>
          </div>
        </div>

        {/* Text Info */}
        <div className="text-white text-[13px]"><p>Free shipping over $199</p></div>
        <div className="text-white text-[13px]"><p>30 days money back</p></div>
        <div className="text-white text-[13px]"><p>100% secure payment</p></div>
      </section>
    </header>
  );
};
